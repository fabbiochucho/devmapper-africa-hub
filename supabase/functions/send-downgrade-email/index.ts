...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Trusted internal callers (eg. the Flutterwave/Paystack refund webhooks)
    // authenticate with the service role key directly, since there's no
    // human user JWT for a server-to-server payment event. Everyone else
    // must be an authenticated admin/platform_admin.
