...
const ALL_QUICK_ACTIONS = [
  ...
  // Guided Workflows (conversational, multi-step - not one-shot templates)
  { label: "Guided Report Submission", icon: <Sparkles className="h-3 w-3" />, context: "general", category: "general",
    prompt: "Walk me through submitting a new project report, one question at a time. Start by asking for the project title and a short description. Wait for my answer before asking the next question (SDG goal, location/country, project status, then optionally cost/budget, timeline dates, and sponsor/funder/contractor). After each answer, briefly confirm what you understood before moving to the next question. Once you have title, description, SDG goal, project status, and location, summarize everything back to me and tell me I'm ready to fill in the Submit Report form with these answers.", requiresProject: false },
  { label: "Guided Certification Application", icon: <Sparkles className="h-3 w-3" />, context: "compliance", category: "general",
    prompt: "Walk me through applying for project certification, one question at a time. Start by asking which verification level I'm targeting (Self Report, Citizen, NGO, Government, or Platform Audit) and why. Wait for my answer before asking about evidence I have available (photos, documents, satellite data, testimonials) and whether my project has completed the earlier SPVF workflow stages. After each answer, briefly confirm what you understood before moving to the next question. Once you understand my evidence and target level, tell me whether I look ready to apply and what's missing if not.", requiresProject: false },
  // General
